...
menu_move_sfx.set_volume(sfx_volume)
...
# ================= GAME =================
game_scene = GameScene(screen, sfx_volume)
...
while running:
    ...
    for event in events:
        ...
        if state == MENU and event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_UP:
                options_selected = (options_selected - 1) % 2
                menu_move_sfx.play()
            elif event.key == pygame.K_DOWN:
                options_selected = (options_selected + 1) % 2
                menu_move_sfx.play()
                ...
                if options_selected == 0:
                    ...
                    menu_move_sfx.set_volume(sfx_volume)
                    game_scene.set_sfx_volume(sfx_volume)
    ...
    if state == MENU:
        menu.draw()
        y = menu.get_selected_y()
        pygame.draw.polygon(
            screen, (0,255,255),
            [(ARROW_X, y), (ARROW_X+20, y+10), (ARROW_X, y+20)]
        )
        ...
        screen.blit(big_font.render("OPTIONS", True, (255,255,255)),
                    (WIDTH//2 - 120, 140))

        labels = ["MUSIC VOLUME", "SFX VOLUME"]
        values = [music_volume, sfx_volume]

        for i in range(2):
            color = (0,255,255) if i == options_selected else (200,200,200)
            screen.blit(font.render(labels[i], True, color),
                        (WIDTH//2 - 160, 260 + i*90))
            pygame.draw.rect(screen, (100,100,100),
                             (WIDTH//2 - 160, 300 + i*90, 320, 10))
            pygame.draw.circle(screen, (0,255,255),
                               (WIDTH//2 - 160 + int(values[i]*320),
                                305 + i*90), 12)
